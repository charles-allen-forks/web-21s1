<template>
  ...
  >
    ...
    <div
      class="booking-seats mt-6"
      :class="{ [screening.layout]: true }"
    >
...
export default class Book extends Vue {
  ...
  async book (): Promise<void> {
    ...
    batch.set(db.collection('users').doc('chaz').collection('tickets').doc(), ticket)
    batch.set(db.collection('screenings').doc(this.screeningSlug), screeningUpdate, { merge: true })
    await batch.commit()
